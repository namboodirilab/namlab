...
int trialCount[numCS];
int correctTrial[numCS];
...
void loop() {
  ...
  // 31 = laser
   ...
   //----- Draw a trial according to ERADE prescription.
    ...
    float minProb=0.15;
    float sumWrongFrac=0;
    float rhoEstim[numCS];
    float assignFrac[numCS];
    float sumProb=0;
    int sumCorrect=0;
    
          for (int temp = 0; temp <numCS; temp++) {    
          sumCorrect=sumCorrect+correctTrial[temp];
              }
      
      for (int temp = 0; temp <numCS; temp++) {    
          wrongFrac[temp]=max(min((1-correctTrial[temp])/(4-sumCorrect),1-minProb),minProb);
      }
      for (int temp = 0; temp <numCS; temp++) {    
          sumWrongFrac=sumWrongFrac+wrongFrac[temp];
  }
        for (int temp = 0; temp <numCS; temp++) {    
          rhoEstim[temp]=wrongFrac[temp]/sumWrongFrac;
  }
        for (int temp = 0; temp <numCS; temp++) {    
          assignFrac[temp]=trialCount[temp]/(CSct+1);
  }
          for (int temp = 0; temp <numCS; temp++) {    
          if (assignFrac[temp]>rhoEstim[temp]){
            trialProb[temp] = alpha* rhoEstim[temp];
          }
          else if (assignFrac[temp]=rhoEstim[temp]){
               trialProb[temp] =rhoEstim[temp];
          }
          else{
            trialProb[temp] =1-alpha*(1-rhoEstim[temp]);
          }
  }

        for (int temp = 0; temp <numCS; temp++) {    
          sumProb=sumProb+trialProb[temp];
  }

          for (int temp = 0; temp <numCS; temp++) {    
          trialProb[temp]=100*trialProb[temp]/sumProb;
  }
...
u=random(100);
    if (u<trialProb[1]){
      cueList[CSct]=0;
        ...
        else if  (u>trialProb[1] && u<trialProb[1]+trialProb[2]){
 cueList[CSct]=1;
        }
         else if  (u>trialProb[1]+trialProb[2] && u<trialProb[1]+trialProb[2]+trialProb[3]){
 cueList[CSct]=2;
        }
        else{
           cueList[CSct]=3;
  ...
  if (ts >= nextcue && ((ITIflag && intervaldistribution < 3) || intervaldistribution > 2) && nextcue != 0) {
    trialCount[CSct]=trialCount[CSct]+1;
    if (secondcue == 0) {
  ...
  if (((!ITIflag && intervaldistribution < 3) || intervaldistribution > 2) && ts >= nextfxdsolenoid && nextfxdsolenoid != 0) { // give fixed solenoid
    ...
    if (intervaldistribution < 3) {
      ...
      if (CSsolenoidcode[2 * cueList[CSct] + numfxdsolenoids] == 35 && CSopentime[2 * cueList[CSct] + numfxdsolenoids] > 0 && u < CSprob[2 * cueList[CSct] + numfxdsolenoids]) {
        ...
        Serial.print('\n');
        correctTrial[CSct] =  correctTrial[CSct] +1;
      }
