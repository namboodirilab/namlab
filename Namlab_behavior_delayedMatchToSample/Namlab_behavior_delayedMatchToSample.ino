...
void loop() {
  ...
  if (((!ITIflag && intervaldistribution < 3) || intervaldistribution > 2) && ts >= nextfxdsolenoid && nextfxdsolenoid != 0) {
    if (intervaldistribution < 3) {
  ...
  if (ts >= nextvacuumOff && nextvacuumOff != 0) { // vacuum offset
    ...
    if (ITIflag == false) {            // if exit was from a trial, move into ITI and set the next cue and bgdsolenoid times
      ...
      for (int temp = 0; temp < numCS; temp++) {
        correctFraction[temp] = correctTrial[temp] / trialCount[temp];
      }

      for (int temp = 0; temp < numCS; temp++) {
        sumCorrect += correctFraction[temp];
      }

      for (int temp = 0; temp < numCS; temp++) {
        wrongFrac[temp] = max(min((1 - correctFraction[temp]) / (4 - sumCorrect), 1 - minProb), minProb);
      }
      for (int temp = 0; temp < numCS; temp++) {
        sumWrongFrac += wrongFrac[temp];
      }
      for (int temp = 0; temp < numCS; temp++) {
